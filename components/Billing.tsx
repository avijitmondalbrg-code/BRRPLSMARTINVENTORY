...
import { FileText, Printer, Save, Eye, Plus, ArrowLeft, Search, Trash2, X, Wallet, IndianRupee, Edit, MessageSquare, Wrench, PackagePlus, CheckCircle2, Download, Calendar } from 'lucide-react';
...
const numberToWords = (amount: number): string => {
  ...
  const convert = (num: number): string => {
    ...
    return convert(Math.floor(amount)) + ' Crore';
  };
...
export const Billing: React.FC<BillingProps> = ({ inventory, invoices = [], patients, advanceBookings = [], onCreateInvoice, onUpdateInvoice, onDelete, logo, signature, userRole }) => {
  ...
  const filteredInvoices = useMemo(() => {
    return (invoices || []).filter(inv => {
      ...
      const matchesStart = !startDate || inv.date >= startDate;
      const matchesEnd = !endDate || inv.date <= endDate;
      return matchSearch && matchesStart && matchesEnd;
    }).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [invoices, searchTerm, startDate, endDate]);

  const exportToCSV = () => {
    const headers = ['Invoice No', 'Date', 'Patient Name', 'Phone', 'Subtotal', 'Tax', 'Grand Total', 'Outstanding', 'Status', 'Serials'];
    const rows = filteredInvoices.map(inv => [
      ...
      inv.paymentStatus,
      `"${inv.items.map(i => i.serialNumber).join('; ')}"`
    ]);
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2"><FileText className="text-[#3159a6]" /> Sales Ledger</h2>
          <div className="flex gap-2 w-full sm:w-auto">
            <button onClick={exportToCSV} className="bg-green-600 text-white px-4 py-2.5 rounded-xl flex items-center gap-2 font-black uppercase text-[10px] tracking-widest shadow-lg hover:bg-green-700 transition"><Download size={16} /> Export CSV</button>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-3 gap-4">
           ...
           <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-xl border border-gray-100 col-span-2">
        ...
        <div className="bg-white rounded-[2rem] shadow-sm overflow-hidden border border-gray-100">
          <div className="overflow-x-auto" id="invoice-printable-area">
            <table className="w-full text-left">
              <thead className="bg-[#3159a6] text-white font-black border-b text-[10px] uppercase tracking-[0.2em]">
                <tr><th className="p-5">Invoice No</th><th className="p-5">Date</th><th className="p-5">Patient</th><th className="p-5">Device Units</th><th className="p-5 text-right">Total</th><th className="p-5 text-right">Outstanding</th><th className="p-5 text-center">Status</th><th className="p-5 text-center print:hidden">Actions</th></tr>
              </thead>
