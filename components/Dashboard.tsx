...
export const Dashboard: React.FC<DashboardProps> = ({ inventory, invoices }) => {
  ...
  const handleGetInsights = async () => {
    ...
    const summary = inventory.map(i => `${i.brand} ${i.model} (${i.status})`).join('\n');
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-6 rounded-xl shadow-sm text-white flex flex-col flex-1">
                ...
                <div className="bg-white/5 rounded-lg p-4 flex-1 overflow-y-auto text-sm leading-relaxed text-gray-200 min-h-[100px]">
                    {insights ? (
                        <div className="whitespace-pre-line">{insights}</div>
                    ) : (
                        <div className="flex flex-col items-center justify-center h-full text-gray-400 text-center">
                            <p>Analyze inventory data for strategic insights.</p>
                        </div>
                    )}
                </div>
            </div>
