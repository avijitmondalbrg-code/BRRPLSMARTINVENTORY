...
import { AdvanceBooking, Patient, UserRole, PaymentRecord } from '../types';
...
import { Search, Plus, X, Printer, IndianRupee, Phone, Briefcase, Trash2, MapPin, CreditCard, Building2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <Briefcase className="text-primary" /> Advance Bookings
          </h2>
          <p className="text-sm text-gray-500">Track patient advance payments.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {bookings.length === 0 ? (
            <div className="col-span-full p-20 text-center bg-white rounded-3xl border border-dashed text-gray-400">
              No advance bookings found. Click "Create Receipt" to add.
            </div>
          ...
          <div key={booking.id} className="bg-white rounded-2xl shadow-sm border p-6 hover:shadow-md transition">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full font-black uppercase border bg-green-50 text-green-700 border-green-200">
                {booking.status}
              </span>
            </div>
            <div className="space-y-2 mb-6">
              <div className="flex items-center gap-2 text-sm text-gray-600"><Phone size={14}/> {booking.phone}</div>
              ...
              <div className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Mode: {booking.paymentMethod} {booking.bankDetails ? `| ${booking.bankDetails}` : ''}</div>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setSelectedBooking(booking)} className="flex-1 py-2 text-xs font-black uppercase bg-teal-50 text-teal-700 rounded-lg flex items-center justify-center gap-2 hover:bg-teal-100">
                <Printer size={14}/> View Receipt
              </button>
              {userRole === 'admin' && (
                <button onClick={() => { if(window.confirm('Delete this record?')) onDeleteBooking(booking.id); }} className="p-2 text-red-400 hover:bg-red-50 rounded-lg">
                  <Trash2 size={18}/>
                </button>
              )}
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in my-8">
            ...
            <form onSubmit={handleAddSubmit} className="p-8 space-y-5">
              <div className="relative">
                <label className="block text-[10px] font-black text-gray-400 uppercase mb-1.5 tracking-widest">Existing Patient Search</label>
                <div className="relative">
                ...
                {showPatientResults && patientSearchTerm && (
                  <div className="absolute z-10 w-full bg-white border rounded shadow-lg max-h-32 overflow-y-auto">
                    {patients.filter(p => p.name.toLowerCase().includes(patientSearchTerm.toLowerCase())).map(p => (
                      <button key={p.id} type="button" onClick={() => handleSelectPatient(p)} className="w-full text-left p-2 hover:bg-teal-50 border-b text-sm font-medium">{p.name}</button>
                    ))}
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase text-gray-400 mb-1 block tracking-widest">Name *</label>
                  <input required className="w-full border-b p-2 outline-none text-sm" value={formData.patientName} onChange={e => setFormData({...formData, patientName: e.target.value})} />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-gray-400 mb-1 block tracking-widest">Phone *</label>
                  <input required className="w-full border-b p-2 outline-none text-sm" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase text-gray-400 mb-1 block tracking-widest">Payment Mode</label>
                  <select 
                    className="w-full border-b p-2 outline-none text-sm bg-white font-bold text-gray-700" 
                ...
                <div>
                  <label className="text-[10px] font-black uppercase text-gray-400 mb-1 block tracking-widest">Received Bank</label>
                  <select 
                    className="w-full border-b p-2 outline-none text-sm bg-white font-bold text-teal-700" 
              ...
              <div>
                <label className="text-[10px] font-black uppercase text-gray-400 mb-1 block tracking-widest">Advance Amount (INR) *</label>
                <input required type="number" className="w-full border-b p-2 outline-none font-black text-teal-700 text-lg" value={formData.amount || ''} onChange={e => setFormData({...formData, amount: Number(e.target.value)})} />
              </div>

              <div>
                <label className="text-[10px] font-black uppercase text-gray-400 mb-1 block tracking-widest">On Account Of / Notes</label>
                <input className="w-full border-b p-2 outline-none text-sm" placeholder="e.g. Model Phonak Lumity booking" value={formData.modelInterest} onChange={e => setFormData({...formData, modelInterest: e.target.value})} />
              </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl my-4 sm:my-8 max-h-[95vh] flex flex-col print:max-h-none print:my-0 print:shadow-none print:rounded-none">
            ...
            <div id="receipt-printable-area" className="p-8 sm:p-12 bg-white overflow-y-auto flex-grow custom-scrollbar print:overflow-visible">
              ...
              <div className="space-y-10 text-sm sm:text-base leading-loose text-gray-800 font-medium py-4">
                ...
                <div className="flex flex-wrap gap-2 items-baseline"><span>By <b>{selectedBooking.paymentMethod}</b> {selectedBooking.bankDetails ? `in <b>${selectedBooking.bankDetails}</b>` : ''} on account of</span><span className="border-b border-dotted border-gray-400 flex-grow px-2 font-bold text-gray-700 italic">{selectedBooking.modelInterest || 'General Device Order'}</span></div>
              </div>
