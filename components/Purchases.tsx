...
import { ShoppingBag, Users, Plus, Search, Calendar, FileText, IndianRupee, Trash2, X, CheckCircle, Store, Tag } from 'lucide-react';
...
export const Purchases: React.FC<PurchasesProps> = ({ vendors, purchases, onAddVendor, onAddPurchase, onDeletePurchase, onDeleteVendor, userRole }) => {
  ...
  const handleSavePurchase = () => {
    if (!selectedVendor || !purchaseForm.invoiceNo || !purchaseForm.serialNumber || !purchaseForm.model) {
        alert("Please select vendor and fill mandatory item details.");
        return;
    }
  ...
  const filteredPurchases = purchases.filter(p => p.invoiceNo.toLowerCase().includes(purchaseSearch.toLowerCase()) || p.vendorName.toLowerCase().includes(purchaseSearch.toLowerCase()) || p.serialNumber.toLowerCase().includes(purchaseSearch.toLowerCase()));
      ...
      {activeTab === 'vendors' ? (
        ...
        <div className="space-y-6 animate-fade-in">
           <div className="flex gap-4">
              <div className="relative flex-1">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                <input className="w-full pl-10 pr-4 py-3 bg-white border rounded-xl outline-none focus:ring-2 focus:ring-primary font-medium" placeholder="Search invoice, vendor or serial..." value={purchaseSearch} onChange={e => setPurchaseSearch(e.target.value)} />
              </div>
                ...
                <tbody className="divide-y text-sm">
                  {filteredPurchases.length === 0 ? (
                    ...
                    <tr key={p.id} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="p-5 font-black text-primary uppercase">{p.invoiceNo}</td>
                      ...
                      <td className="p-5">
                        ...
                        <p className="text-[9px] text-teal-600 font-mono font-bold tracking-widest mt-1">S/N: {p.serialNumber}</p>
                      </td>
            ...
            <div className="p-8 space-y-6">
               ...
               <div className="bg-slate-50 p-6 rounded-3xl border-2 border-slate-100 space-y-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[9px] font-black uppercase text-gray-400 ml-1">Serial Number (S/N) *</label>
               ...
               <button onClick={handleSavePurchase} className="w-full bg-slate-900 text-white py-5 rounded-[2.5rem] font-black uppercase tracking-[0.3em] shadow-2xl hover:bg-black transition active:scale-95 text-[10px] flex items-center justify-center gap-3">
                  <Store size={18}/> Commit Purchase & Push to Stock
               </button>
            </div>
          </div>
        </div>
