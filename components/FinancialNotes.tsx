...
export const FinancialNotes: React.FC<FinancialNotesProps> = ({ type, notes, patients, invoices, onSave, onDelete, logo, signature, userRole }) => {
      ...
      <div id="invoice-printable-area" className="bg-white rounded-xl shadow-2xl p-12 border relative overflow-hidden animate-fade-in">
        <div className="flex justify-between items-start border-b-2 border-gray-800 pb-8 mb-8">
            ...
            <div className="text-right">
                ...
                <p className="text-xs text-gray-400 font-bold uppercase tracking-widest">Date: {note.date}</p>
            </div>
        </div>
        <div className="grid grid-cols-2 gap-12 mb-10 text-sm">
            ...
            <div className="text-right flex flex-col justify-center"><div><h4 className="text-[10px] font-black uppercase text-gray-400 mb-0.5">Note Amount</h4><p className={`text-3xl font-black ${note.type === 'CREDIT' ? 'text-red-600' : 'text-blue-600'}`}>₹{note.amount.toLocaleString('en-IN')}</p></div></div>
        </div>
        <div className="bg-gray-50 p-6 rounded-2xl border mb-10"><h4 className="text-[10px] font-black uppercase text-gray-400 mb-2 border-b">Adjustment Reason / Remarks</h4><p className="font-medium text-gray-700 leading-relaxed">{note.reason}</p></div>
        <div className="flex justify-between items-end mt-20">
            <div><p className="text-[8.5px] text-gray-400 font-bold uppercase tracking-widest">Computer Generated Document • Subject to Kolkata Jurisdiction</p></div>
  ...
  if (viewMode === 'view' && viewNote) {
      return (<div className="max-w-4xl mx-auto space-y-6"><div className="flex justify-between items-center print:hidden"><button onClick={() => setViewMode('list')} className="flex items-center gap-2 text-gray-600 hover:text-teal-700 font-bold transition"><ArrowLeft size={20} /> Back to List</button><button onClick={() => window.print()} className="bg-teal-700 text-white px-6 py-2 rounded-lg shadow flex items-center gap-2 hover:bg-teal-800"><Printer size={20} /> Print Note</button></div>{renderDocument(viewNote)}</div>);
  }
  ...
  return (
      <div className="max-w-xl mx-auto bg-white p-10 rounded-3xl shadow-xl border border-gray-100 animate-fade-in"><h2 className="text-2xl font-black text-gray-800 uppercase tracking-tight mb-8">New {type} Note</h2><div className="space-y-6">
          <div className="relative"><label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1">Select Patient</label><input className="w-full border-2 border-gray-100 p-3 rounded-xl focus:border-teal-500 outline-none transition-all" placeholder="Search by name..." value={patientSearchTerm} onChange={e=>{setPatientSearchTerm(e.target.value); setShowPatientResults(true);}} />{showPatientResults && patientSearchTerm && (<div className="absolute z-10 w-full bg-white border rounded shadow-lg max-h-40 overflow-y-auto">{patients.filter(p=>p.name.toLowerCase().includes(patientSearchTerm.toLowerCase())).map(p=>(<button key={p.id} onClick={()=>{setSelectedPatient(p); setShowPatientResults(false); setPatientSearchTerm(p.name);}} className="w-full text-left p-3 hover:bg-teal-50 border-b text-sm font-bold">{p.name}</button>))}</div>)}</div>
          {selectedPatient && <div className="p-4 bg-teal-50 text-teal-700 rounded-xl text-sm font-black uppercase tracking-widest border border-teal-100">Selected: {selectedPatient.name}</div>}
          <div><label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1">Adjustment Amount (INR)</label><input type="number" className="w-full border-2 border-gray-100 p-3 rounded-xl font-black text-xl outline-none focus:border-teal-500" value={amount || ''} onChange={e=>setAmount(Number(e.target.value))} /></div>
          <div><label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1">Reason for Issuance</label><textarea className="w-full border-2 border-gray-100 p-3 rounded-xl outline-none focus:border-teal-500 h-32 resize-none" value={reason} onChange={e=>setReason(e.target.value)} /></div>
          <div className="flex gap-4 pt-6"><button onClick={()=>setViewMode('list')} className="flex-1 py-4 border-2 border-gray-200 rounded-2xl font-black uppercase tracking-widest hover:bg-gray-50 transition">Cancel</button><button onClick={handleSave} className="flex-1 py-4 bg-primary text-white rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-teal-900/20 hover:bg-teal-800 transition">Confirm Note</button></div>
      </div></div>
  );
};