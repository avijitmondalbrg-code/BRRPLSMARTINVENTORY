  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 z-[70] flex items-center justify-center p-4 overflow-y-auto print:p-0 print:bg-white print:absolute print:inset-0">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden animate-fade-in print:shadow-none print:w-full print:rounded-none">
        <div className="bg-teal-700 p-4 flex justify-between items-center text-white print:hidden"><h3 className="font-bold">Receipt Preview</h3><div className="flex gap-2"><button onClick={handlePrint} className="bg-white text-teal-700 px-4 py-1.5 rounded-lg text-sm font-bold flex items-center gap-2"><Printer size={16} /> Print</button><button onClick={onClose}><X size={24} /></button></div></div>
        <div id="receipt-printable-area" className="p-10 bg-white">
            <div className="flex justify-between items-start border-b-2 border-gray-800 pb-8 mb-8">
                ...
                <div className="text-right"><div className="border-2 border-gray-800 px-4 py-1 inline-block mb-3"><h2 className="text-lg font-black uppercase tracking-widest">Money Receipt</h2></div><p className="text-xs font-bold text-gray-600">No: {payment.id}</p><p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Date: {new Date(payment.date).toLocaleDateString('en-IN')}</p></div>
            </div>
