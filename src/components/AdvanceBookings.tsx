...
import { Search, Plus, X, Printer, IndianRupee, Phone, Briefcase, Trash2, MapPin, Download } from 'lucide-react';
...
export const AdvanceBookings: React.FC<AdvanceBookingsProps> = ({ bookings, patients, onAddBooking, onDeleteBooking, userRole, logo, signature }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedBooking && (
        <div className="fixed inset-0 bg-black/80 z-[130] flex items-center justify-center p-4 overflow-y-auto animate-fade-in backdrop-blur-xl print:bg-white print:p-0 print:block">
          <div className="bg-white rounded-[3rem] w-full max-w-2xl overflow-hidden shadow-[0_50px_100px_rgba(0,0,0,0.5)] my-8 max-h-[95vh] flex flex-col border-4 border-white print:max-h-none print:my-0 print:shadow-none print:rounded-none print:border-0 print:w-full print:max-w-none">
            <div className="bg-primary p-5 flex justify-between items-center text-white sticky top-0 z-20 print:hidden flex-shrink-0">
              <h3 className="font-black uppercase tracking-widest text-xs ml-4">Receipt Digital Preview</h3>
              <div className="flex gap-3 mr-4">
                <button onClick={() => window.print()} className="bg-white text-primary px-6 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:bg-blue-50 transition-all active:scale-95">
            ...
            <div id="invoice-printable-area" className="p-12 sm:p-20 bg-white overflow-y-auto flex-grow custom-scrollbar print:overflow-visible print:p-[10mm] relative">
              ...
              <div className="flex justify-between border-b-4 border-gray-900 pb-12 mb-12 relative z-10">
                ...
                <div className="text-right">
                  <div className="bg-primary text-white px-5 py-2 inline-block mb-4 rounded-xl">
              ...
              <div className="space-y-12 text-sm sm:text-lg leading-loose text-gray-900 font-bold py-6 relative z-10">
                <div className="flex flex-wrap gap-4 items-baseline"><span>Received with thanks from Mr./Mrs./Ms.</span><span className="border-b-2 border-gray-100 flex-grow px-4 font-black text-primary text-xl sm:text-3xl uppercase tracking-tighter">{selectedBooking.patientName}</span></div>
                <div className="flex flex-wrap gap-4 items-baseline"><span>Residing at</span><span className="border-b-2 border-gray-100 flex-grow px-4 font-black text-gray-800 uppercase tracking-tight text-xs sm:text-sm">{selectedBooking.address || 'Not Registered'}</span></div>
                <div className="flex flex-wrap gap-4 items-baseline"><span>A sum of Indian Rupees</span><span className="border-b-2 border-gray-100 flex-grow px-4 font-black text-primary text-xl sm:text-3xl tracking-tighter">₹ {selectedBooking.amount.toLocaleString('en-IN')} /-</span></div>
              ...
              <div className="mt-28 flex flex-col sm:flex-row justify-between items-center sm:items-end gap-16 relative z-10">
                <div className="bg-primary p-8 sm:p-10 rounded-[2.5rem] text-center flex flex-col gap-2 w-full sm:w-auto shadow-2xl relative">
